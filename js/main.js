...
const translations = {
    ...
    fr: {
        ...
        "why_choose_us_description": "Chez ZeroOne Studio, nous combinons innovation, clarté et un engagement authentique envers ceux qui nous font confiance. Notre mode de fonctionnement repose sur des solutions technologiques simples mais puissantes, conçues pour simplifier les processus, générer des résultats et s'adapter aux besoins de chaque client. Ils nous choisissent parce que nous comprenons que derrière chaque défi numérique se cachent des personnes à la recherche de réponses concrètes, de relations honnêtes et d'une expérience sans tracas.",
        "about_us_description": "Dans le cadre de notre identité, l'icône intègre subtilement le binaire pour 'simple', reflétant notre objectif d'offrir des expériences numériques simplifiées - un élément qui invite les gens à découvrir ce qu'est ZeroOne Studio.",
    ...
    it: {
        ...
        "why_choose_us_description": "In ZeroOne Studio combiniamo innovazione, chiarezza e un impegno autentico verso chi si fida di noi. Il nostro modo di lavorare si basa su soluzioni tecnologiche semplici ma potenti, progettate per semplificare i processi, ottenere risultati e adattarsi alle esigenze di ogni cliente. Ci scelgono perché capiamo che dietro ogni sfida digitale ci sono persone alla ricerca di risposte concrete, relazioni oneste e un'esperienza senza problemi.",
        "about_us_description": "Come parte della nostra identità, l'icona integra sottilmente il binario per 'semplice', riflettendo il nostro scopo di offrire esperienze digitali semplificate - un elemento che invita le persone a scoprire cos'è ZeroOne Studio.",
...
function changeLanguage(newLang) {
    ...
    applyPlaceholderTranslations(newLang);
}
...
function applyPlaceholderTranslations(lang) {
  ...
  document.querySelectorAll("[data-translate-placeholder]").forEach(el => {
    const key = el.getAttribute("data-translate-placeholder");
    if (placeholderTranslations[lang] && placeholderTranslations[lang][key]) {
      el.setAttribute("placeholder", placeholderTranslations[lang][key]);
    }
  });
}
